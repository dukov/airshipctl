...
airshipctl config set-context e2e --%v=kube-system --%v=manifest --%v=auth-info
...
func runSetContext(o *config.ContextOptions, airconfig *config.Config) (bool, error) {
	...
	// Safe to ignore the error. Simple means I didnt find the Context
	if context == nil {
		if o.CurrentContext {
			return contextWasModified,
				fmt.Errorf("Desired Current Context %s was not found as a defined context in the configuration.\n",
					o.Name)
		}
		_, err := airconfig.AddContext(o)
		if err != nil {
			return contextWasModified, err
		}
		contextWasModified = false
		...
		// Lets updated it and be done.
		if o.CurrentContext {
			airconfig.CurrentContext = o.Name
		} else {
			// Context exists, lets update
			_, err := airconfig.ModifyContext(context, o)
			if err != nil {
				return contextWasModified, err
			}
		}
